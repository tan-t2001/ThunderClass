...
bool TeacherProcess::EndClass() {
    ...
    //教师端下课时将注意力vector存到文件中
    SaveToFile("Attention.txt");
...
/*************************************************************************
【函数名称】TeacherProcess::SendPrivateChat
【函数功能】私发消息
【参数】入口参数，消息内容QString、接收方connection号
【返回值】是否发送成功
*************************************************************************/
bool TeacherProcess::SendPrivateChat(const QString& ChatStr,const unsigned int& ReceiverNum) {
    if (!m_bIsInClass) {
        return m_bIsInClass;
    }
    std::string ToSend = ChatStr.toStdString();
    Message Chat(MSG_STRING, (unsigned char*)ToSend.c_str(), ToSend.length());
    m_pServer->SendTo(ReceiverNum,Chat);
    qDebug() << "TP: send to all message: " << ChatStr;
    return m_bIsInClass;
}
...
void TeacherProcess::RecvAttention(const unsigned int& Idx,const Message& Msg) {
    string TempAttention = m_Names[Idx] + "    " + Message::ToAttention(Msg);
    m_Attention.push_back(TempAttention);
}

void TeacherProcess::SaveToFile(const string& FileName) {
    ofstream OutFile(FileName.c_str());
    OutFile << "序号    用户名    注意力百分比（%）" << endl;
    for (unsigned int i = 0; i < m_Attention.size(); i++) {
        OutFile << i << "      "<< m_Attention[i] << endl;
    }
}
...
DWORD WINAPI TeacherProcess::MsgReceiveThread(LPVOID lpParameter) {
    ...
    while (pProcess->IsInClass) {
        ...
        if (pProcess->m_pServer->GetAndRemoveFirstMessage(TempPair)) {
            ...
            //如果是文字信息，就通知界面更新
            if (Type == MSG_STRING) {
                qDebug() << "TP: recieved a string from connection " << TempPair.first;
                emit pProcess->RecvChat(QString::fromStdString(Message::ToString(TempPair.second)));
            }
            //如果是注意力消息，就存入注意力vector中
            else if (Type == MSG_ATTENTION) {
                qDebug() << "TP: recieved attention message from connection " << TempPair.first;
                emit pProcess->RecvAttention(TempPair.first,TempPair.second);
            }
            //如果是用户名密码的登录消息
