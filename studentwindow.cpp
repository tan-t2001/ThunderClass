    ...
    ui(new Ui::StudentWindow)
{
    ...
    connect(m_pTimer, SIGNAL(timeout()), this, SLOT(ShowAttentionTime()));
    connect(m_pTimer, SIGNAL(timeout()), this, SLOT(ShowTime()));
    m_pTimer->start();
    m_pAttentionTimer = new QTimer();
    m_pAttentionTimer->setInterval(200);
    connect(m_pAttentionTimer, SIGNAL(timeout()), this, SLOT(AttentionTime()));
    m_pAttentionTimer->start();
    //只显示关闭窗口按钮，没有？按钮
...
StudentWindow::~StudentWindow()
{
    ...
    delete m_pTimer;
    m_pTimer = nullptr;
    m_pAttentionTimer->stop();
    disconnect(m_pAttentionTimer, SIGNAL(timeout()), this, SLOT(AttentionTime()));
    delete m_pAttentionTimer;
    m_pAttentionTimer = nullptr;
...
void StudentWindow::ShowTime() {
    ui->label->setText(QString::number(m_pProc->m_pTime));
}

void StudentWindow::AttentionTime() {
    //上课中，窗口是激活状态时计时
    if (m_pProc->IsInClass && this->isActiveWindow()) {
        m_pProc->m_pAttention += 200;
    }
}

void StudentWindow::ShowAttentionTime() {
    ui->label_2->setText(QString::number(m_pProc->m_pAttention));
}
