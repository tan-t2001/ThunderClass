...
【返回值】文本消息对象
*************************************************************************/
Message Message::FromQuiz(const string& aQuestionAndOptions) {
    return Message(MSG_QUIZ, (const unsigned char*)aQuestionAndOptions.c_str(), aQuestionAndOptions.length());
}

/*************************************************************************
【函数名称】Message::FromAttention
【函数功能】产生一个注意力比例文本消息
【参数】入口参数，注意力比例百分数（已转换为string）
【返回值】文本消息对象
*************************************************************************/
Message Message::FromAttention(const string& aAttention) {
    return Message(MSG_ATTENTION, (const unsigned char*)aAttention.c_str(), aAttention.length());
}
...
/*************************************************************************
【函数名称】Message::ToQuiz
【函数功能】从题目消息还原一个题目文本对象
【参数】入口参数，要还原的消息
【返回值】题目文本对象
*************************************************************************/
string Message::ToQuiz(const Message& Msg) {
    //开辟临时缓冲区
    unsigned char* pBuffer = new unsigned char[Msg.Size];
    //消息类型和解码后长度临时变量
    MessageType type;
...
/*************************************************************************
【函数名称】Message::ToAttention
【函数功能】从注意力消息还原一个注意力文本对象
【参数】入口参数，要还原的消息
【返回值】注意力文本对象
*************************************************************************/
string Message::ToAttention(const Message& Msg) {
    //开辟临时缓冲区
    unsigned char* pBuffer = new unsigned char[Msg.Size];
    //消息类型和解码后长度临时变量
    MessageType type;
